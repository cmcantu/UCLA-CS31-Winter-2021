...
namespace cs31
{
    ...
    // TODO: called to allow the human player to randomly roll its dice ok!
    void BeatThat::humanPlay( )
    {
        mHuman.roll();
    }
    
    // TODO: called to force a certain human player roll ok!
    void BeatThat::humanPlay( Die d1, Die d2 )
    {
        mHuman.roll(d1, d2);
    }

    // TODO: called to allow the computer player to randomly roll its dice ok!
    void BeatThat::computerPlay( )
    {
        mComputer.roll();
    }

    // TODO: called to force a certain computer player roll ok!
    void BeatThat::computerPlay( Die d1, Die d2 )
    {
        mComputer.roll(d1,d2);
    }
        
    // TODO: called by isGameOver( ) to figure out the current state of the game ok!
    ...
    {
        GAMEOUTCOME result = GAMEOUTCOME::GAMENOTOVER; //initalize with game not over
        if (mTurnCount == 5) { //had 5 rounds, so reach game over
            if (mHuman.getRoundsWon() > mComputer.getRoundsWon()) //human won more rounds
                result = GAMEOUTCOME::HUMANWONGAME;
            else if (mHuman.getRoundsWon() < mComputer.getRoundsWon()) //computer won more rounds
                result = GAMEOUTCOME::COMPUTERWONGAME;
            else if (mHuman.getRoundsWon() == mComputer.getRoundsWon()) //human and computer tied rounds
